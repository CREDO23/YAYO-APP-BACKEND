...
const getAllPartners = async (req, res, next) => {
    ...
    if (isValidObjectId(id)) {
      if (!utils.isObjctEmpty(updated)) {
        try {
          ...
          if (updatedPartner) {
            res.json({ message: "Updated successful", updatedPartner });
          } else {
  ...
  const deletePartner = async (req, res, next) => {
    ...
    if (isValidObjectId(id)) {
      try {
        const deletedPartner = await Partner.findByIdAndDelete(id).catch((error) =>
          next(error)
        );
  
        if (deletedPartner) {
          res.json({ message: "Deleted successful", deletedPartner });
        } else {
          throw createError.NotFound("This partner doesn't exist");
        }
      } catch (error) {
  ...
  module.exports = { getAllPartners, getPartner, updatePartner, createPartner, deletePartner };
